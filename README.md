...
Python tools I build and use to analyze historical price data and strategy tester results, published so you can read them, run them, and change them.

These are working tools, not a product. Nothing here is polished. If something breaks, the code is short enough that you can open it and see why, which is the point.

Built and demonstrated on the [AlgoCoach](https://www.youtube.com/@algocoach) YouTube channel. Downloads and walkthroughs at [algocoach.com](https://algocoach.com).

## What's in here

| File | What it does |
| --- | --- |
| `Best_Times_to_Initiate_a_Trade_Checker_1_1.py` | Scans historical M1 data and reports, for every 15 minute slot in the trading day, how often price travelled a given distance in your favour before it travelled a given distance against you. |
...
Download the repo (green **Code** button, then **Download ZIP**) and unzip it somewhere sensible, for example `C:\AlgoCoach\tools`.

Open that folder in VS Code, open a terminal, and create an isolated environment so you don't disturb any other Python on your machine:
...
The script looks for CSV files sitting in the same folder as the script itself, so it will find `NAS100sample.csv` with no configuration. Charts and a plain text summary are written to that same folder.

To use your own data, export M1 history from your platform to CSV and drop it in beside the script. Match the column layout of `NAS100sample.csv`. The target distance, stop distance, and other settings are plain variables near the top of the file. Change them and run it again.
...
4. **Broker daily break shows up as missing data.** Slots around the platform's daily maintenance window will be thin or empty. That is the data, not the market.
...
This repository is for educational and informational purposes only. I am not a financial advisor, and nothing here is financial, investment, or trading advice. Trading forex, futures, indices, and CFDs carries a substantial risk of loss and is not suitable for everyone. My results are my own and do not guarantee or predict your results, and past performance is not indicative of future performance. Any tools discussed here are for educational use; you are solely responsible for your own trading decisions. FTMO, MetaQuotes, and other referenced brands are the property of their respective owners; references reflect my own experience and do not imply endorsement or affiliation.
